...
class DiscussionDetailsPage extends StatefulWidget {
  ...
  DiscussionDetailsPage(
      {Key? key, required this.discussion, required this.discussionId})
      : super(key: key);
...
class _DiscussionDetailsPageState extends State<DiscussionDetailsPage> {
  ...
  Future<void> _deleteDiscussion() async {
    ...
    try {
      DatabaseReference discussionRef = FirebaseDatabase.instance
          .ref()
          .child('Discussions/${widget.discussionId}');
...
class ReplySection extends StatefulWidget {
  ...
  ReplySection({Key? key, required this.discussionId, required this.createdBy})
      : super(key: key);
...
class _ReplySectionState extends State<ReplySection> {
  ...
  void sendReply(String replyText) async {
    ...
    final DatabaseReference discussionRef = FirebaseDatabase.instance
        .ref()
        .child('Discussions/${widget.discussionId}/replies');
  ...
  void deleteReply(String replyKey) async {
    final DatabaseReference replyRef = FirebaseDatabase.instance
        .ref()
        .child('Discussions/${widget.discussionId}/replies/$replyKey');
    await replyRef.remove();
  }

  @override
  Widget build(BuildContext context) {
    final DatabaseReference repliesRef = FirebaseDatabase.instance
        .ref()
        .child('Discussions/${widget.discussionId}/replies');
    return Column(
      children: [
        ...
        StreamBuilder(
          ...
          builder: (context, AsyncSnapshot<DatabaseEvent> snapshot) {
            if (snapshot.hasData &&
              ...
              Map<dynamic, dynamic> repliesMap = Map<dynamic, dynamic>.from(
                  snapshot.data!.snapshot.value as Map);
              List<dynamic> replies =
                  repliesMap.entries.map((e) => e.value).toList();
                  ...
                  return FutureBuilder<Map<String, dynamic>?>(
                    ...
                    builder: (context,
                      ...
                      if (userSnapshot.connectionState ==
                        ...
                        bool canDelete = currentUser?.uid == reply['userId'] ||
                            currentUser?.uid == widget.createdBy;
